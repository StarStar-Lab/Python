...
# MOD by 肉蛋葱鸡 自用贴纸包命名和骚话


@listener(outgoing=True, command="tnt",
...
async def sticker(context):
    ...
    if photo:
        ...
        pack_title = f"东半球最好的贴纸包{pack}"
        command = '/newpack'
        file = BytesIO()

        if not animated:
            await context.edit("调整🐔🐔大小中 . . .")
            image = await resize_image(photo)
            file.name = "sticker.png"
            image.save(file, "PNG")
        else:
            pack_name = f"{user.username}_gif_{pack}"
            pack_title = f"东半球最好的GIF包{pack}"
            command = '/newanimated'
...
A pack can't have more than 120 stickers at the moment.":
                    ...
                    pack_name = f"{user.username}_{pack}"
                    pack_title = f"东半球最好的贴纸包{pack}"
                    await context.edit("切换到东半球最好的 " + str(pack) +
                                       " 上一个贴纸包已偷满 . . .")
                    await conversation.send_message(pack_name)
                    chat_response = await conversation.get_response()
                    if chat_response.text == "Invalid pack selected.":
                        await add_sticker(conversation, command, pack_title, pack_name, animated, message,
                                          context, file, emoji)
                        await context.edit(
                            f"上一秒是你在秀 下一秒就是我在秀 ！成辣，偷就完了嗷 [奥力给](t.me/addstickers/{pack_name}) ! ---An Old Dutch",
                            parse_mode='md')
                        return
